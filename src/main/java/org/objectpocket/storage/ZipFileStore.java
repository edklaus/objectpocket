...
public class ZipFileStore extends FileStore {
	...
	protected BufferedReader getBufferedReader(String filename) throws IOException {
		...
		throw new IOException("Couöl not read file " + filename + " in zip " + zipfile);
	}
	...
	private void initZipFile() throws IOException {
		...
		if (!f.getParentFile().exists()) {
			throw new IOException("Directory does not exist and could not be created. " + f.getParent());
		}
		...
		try (ZipInputStream zipIn = new ZipInputStream(new FileInputStream(zipfile))) {
			...
			throw new IOException("Could not read index file from " + this.zipfile, e);
		}
		if (!indexFileFound) {
	...
	protected void readIndexFile() throws IOException {
		...
		throw new IOException("Could not parse index file data to index object. " + this.zipfile);
	}
